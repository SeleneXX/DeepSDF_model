...
class SdfDataset(data.Dataset):
    def __init__(self, points=None, normals=None, phase='train', args=None):
        ...
        if self.phase == 'test':
            ...
            if phase == 'val':
                ...
                self.samples_sdf = np.random.normal(0, 0.05, size=(self.points.shape[0] * 80, 1))
                repeat_point = np.repeat(self.points, 80).reshape(3, self.points.shape[0] * 80).T
                repeat_norm = np.repeat(self.normals, 80).reshape(3, self.points.shape[0] * 80).T
                self.samples_xyz = repeat_point + repeat_norm * self.samples_sdf
                # ***********************************************************************
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'val':
            ...
            gt_sdf = np.random.normal(0, 0.05, size=(this_bs, 1))
            repeat_point = np.repeat(self.points, 80).reshape(3, self.points.shape[0] * 80).T[start_idx: end_idx, :]
            repeat_norm = np.repeat(self.normals, 80).reshape(3, self.points.shape[0] * 80).T[start_idx: end_idx, :]
            xyz = repeat_point + repeat_norm * gt_sdf
            # ***********************************************************************
