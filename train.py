...
def train(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        p_si = model.forward(xyz_tensor)
        si = data['gt_sdf']
        loss = torch.abs(torch.clamp(p_si, -args.clamping_distance, args.clamping_distance) - torch.clamp(si, -args.clamping_distance, args.clamping_distance))
        loss_sum += torch.sum(loss)
        loss_count += loss.shape[0]
...
def val(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        with torch.no_grad():
            ...
            p_si = model.forward(xyz_tensor)
            si = data['gt_sdf']
            loss = torch.abs(torch.clamp(p_si, -args.clamping_distance, args.clamping_distance) - torch.clamp(si,
                                                                                                              -args.clamping_distance,
                                                                                                              args.clamping_distance))
            loss_sum += torch.sum(loss)
            loss_count += loss.shape[0]
            # xyz_tensor = data['xyz'].to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--evaluate", default='true', action="store_true", help="Activate test mode - Evaluate model on val/test set (no training)")

    # paths you may want to adjust, but it's better to keep the defaults
    parser.add_argument("--input_pts", default="data/bunny-1000.pts", type=str, help="Folder to save checkpoints")
    parser.add_argument("--save_file_name", default="bunny_1000.html", type=str, help="Folder to save testing visualization")
